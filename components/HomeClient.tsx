...
export default function HomeClient() {
  const { cancelSearch, phase, primaryRole, region, resetDemo, searchSeconds, setPrimaryRole, setRegion, startSearch } = useMatchmaking();
      ...
      <div className="space-y-8">
        <section id="home" className="grid items-center gap-8 py-10 lg:grid-cols-[1.15fr_.85fr]">
          <div className="space-y-7">
            ...
            <h1 className="max-w-4xl text-5xl font-black leading-tight tracking-tight md:text-7xl">{config?.content.heroTitle || 'Competitive Dota 2 with verified trust.'}</h1>
            <p className="max-w-2xl text-lg text-zinc-300">{config?.content.heroText || 'Выберите регион и основную роль, запустите поиск и пройдите полный mock-сценарий матчмейкинга TRUST.'}</p>
